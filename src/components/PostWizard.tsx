...
const STEPS = ["Name", "Dates", "Format", "Idea", "Inspiration"] as const;
...
export function PostWizard(props: Props) {
  ...
  return (
    <div className="flex min-h-dvh flex-col">
          ...
          {STEPS.map((_, i) => (
            <div
              ...
              className={`h-1.5 flex-1 rounded-full transition-colors ${
                i <= step ? "bg-[var(--color-post)]" : "bg-white/10"
              }`}
            />
          ))}
        </div>
      ...
      <div className="flex-1 px-5 pt-4 pb-28 flex flex-col">
        {step === 0 && (
          <StepShell title="What's this post called?">
            <input
              autoFocus
              type="text"
              value={form.name}
        ...
        {step === 1 && (
          <StepShell title="When's it happening?">
        ...
        {step === 2 && (
          <StepShell title="What format is it?">
            <div className="grid grid-cols-2 gap-2.5">
              {POST_TYPES.map((t) => {
                ...
                return (
                  ...
                  >
                    <span
                      className="block h-2.5 w-2.5 rounded-full mb-2.5"
        ...
        {step === 3 && (
          <StepShell title="What's the idea?">
            <textarea
              autoFocus
              value={form.idea}
        ...
        {step === 4 && (
          <StepShell title="Any inspiration?" subtitle="Optional">
            <textarea
              autoFocus
        ...
        {error && <p className="text-sm text-[var(--color-shoot)] mt-4">{error}</p>}
      </div>
      ...
      <div className="fixed bottom-0 inset-x-0 px-5 pb-6 pt-4 bg-gradient-to-t from-[var(--color-bg)] via-[var(--color-bg)] to-transparent">
        ...
        >
          {saving ? "Saving…" : isLast ? (isEdit ? "Save changes" : "Add post") : "Next"}
        </button>
      </div>
    </div>
  );
...
function StepShell({
  ...
  return (
    <div>
      <h1 className="font-display text-3xl tracking-wide mb-0.5">{title}</h1>
      {subtitle && <p className="text-sm text-white/40 mb-4">{subtitle}</p>}
