...
import { markPaidBack, removeReceivable } from "../receivables/actions";
import { formatMoney } from "@/lib/format";
import type { Currency, Receivable } from "@/lib/types";

export default function OwedEntryRow({
  receivable,
  personName,
  accountId,
  currency,
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] px-3 py-2.5">
      <div className="min-w-0">
        <Link
          href={`/day-to-day/receivables/${receivable.transactionId}`}
          className="text-sm font-medium truncate hover:text-white/80 transition-colors"
        >
          {transactionNote || "Expense"}
        </Link>
        <p className="text-xs text-[var(--color-fg-dim)] truncate">
          {transactionDate} · {isOutstanding ? "Outstanding" : "Paid back"}
        </p>
      </div>
