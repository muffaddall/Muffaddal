...
import { editElevatePurchase, removeElevatePurchase, toggleElevatePurchasePaid } from "./actions";
import { formatMoney } from "@/lib/format";
import type { ElevatePurchase } from "@/lib/types";

export default function ElevatePurchaseRow({ purchase }: { purchase: ElevatePurchase }) {
  ...
  const [state, formAction, pending] = useActionState(async (
    ...
    const result = await editElevatePurchase(prev, formData);
    ...
    <li className="flex items-center justify-between gap-3 rounded-lg px-2 py-2 hover:bg-white/5 transition-colors">
        ...
        <label className="flex shrink-0 items-center" title={purchase.paid ? "Paid" : "Mark as paid"}>
          <input
            ...
            onChange={(e) => {
              ...
              startTogglePaid(() => toggleElevatePurchasePaid(purchase.id, paid));
            }}
      ...
      <div className="flex shrink-0 items-center gap-3">
        ...
        <button
          type="button"
          disabled={isDeleting}
          onClick={() => startDelete(() => removeElevatePurchase(purchase.id))}
