...
import { editSavingsPurchase, removeSavingsPurchase, toggleSavingsPurchasePaid } from "./actions";
import { formatMoney } from "@/lib/format";
import type { SavingsPurchase } from "@/lib/types";

export default function SavingsPurchaseRow({ purchase }: { purchase: SavingsPurchase }) {
  ...
  const [state, formAction, pending] = useActionState(async (
    ...
    const result = await editSavingsPurchase(prev, formData);
    ...
    <li className="flex items-center justify-between gap-3 rounded-lg px-2 py-2 hover:bg-white/5 transition-colors">
        ...
        <label className="flex shrink-0 items-center" title={purchase.paid ? "Paid" : "Mark as paid"}>
          <input
            ...
            onChange={(e) => {
              ...
              startTogglePaid(() => toggleSavingsPurchasePaid(purchase.id, paid));
            }}
      ...
      <div className="flex shrink-0 items-center gap-3">
        ...
        <button
          type="button"
          disabled={isDeleting}
          onClick={() => startDelete(() => removeSavingsPurchase(purchase.id))}
